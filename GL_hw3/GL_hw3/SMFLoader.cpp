...
void SMFLoader::load(char* filename){
	...
	while (!fin.eof()){
		...
		/*case 'n'://n
			...
			{
				float* tmp = normal;
				color = new float[normalSize * 3 * 4];
...
float* SMFLoader::getFace(int index)
{
	...
	putin[0] = vertex[v * 3];
	putin[1] = vertex[v * 3 + 1];
	putin[2] = vertex[v * 3 + 2];

	v = face[index * 3 + 1] - 1;
	putin[3] = vertex[v * 3];
	putin[4] = vertex[v * 3 + 1];
	putin[5] = vertex[v * 3 + 2];

	v = face[index * 3 + 2] - 1;
	putin[6] = vertex[v * 3];
	putin[7] = vertex[v * 3 + 1];
	putin[8] = vertex[v * 3 + 2];
...
/*float* SMFLoader::getFacenormal(int index)
{
	//float* tmp;
